...
export class Form extends Component<{}, any> {
  ...
  async handleFormSubmit(e: SyntheticEvent) {
    e.preventDefault();
    if (
    ...
    const apiPayload = {
      ...
      password: this.state.email.value,
    };
    ...
    try {
      const user = await fetch("https://login.com", requestOptions);
    } catch (error: any) {
      this.setState((prev: prevState) => ({
        ...prev,
        user: {
  ...
  render() {
    return (
      <main className={Styles.form_container}>
        <header>
          <h1>Form validation on input</h1>
        </header>
        <div>
          <form className={Styles.form} onSubmit={this.handleFormSubmit}>
            ...
            <button className={Styles.submit_button}>
              {this.state.user.loading ? "loading..." : "Login"}
            </button>
          </form>
          {
            this.state.user.error && <p className={Styles.error}>{ this.state.user.error}</p>
          }
        </div>
      </main>
    );
